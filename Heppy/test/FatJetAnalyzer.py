...
class FatJetAnalyzer( Analyzer ):
    """Taken from RootTools.FatJetAnalyzer, simplified, modified, added corrections    """
    def __init__(self, cfg_ana, cfg_comp, looperName):
        super(FatJetAnalyzer,self).__init__(cfg_ana, cfg_comp, looperName)
    ...
    def declareHandles(self):
        super(FatJetAnalyzer, self).declareHandles()
    ...
    def beginLoop(self, setup):
        super(FatJetAnalyzer,self).beginLoop(setup)
    ...
    def process(self, event):
        ...
        if self.cfg_ana.copyJetsByValue: 
          import ROOT
          allJets = map(lambda j:Jet(ROOT.pat.Jet(j)), self.handles['jets'].product()) #copy-by-value is safe if FatJetAnalyzer is ran more than once
        else: 
...
#        print "after. rho",self.rho,self.cfg_ana.collectionPostFix,'allJets len ',len(allJets),'pt', [j.pt() for j in allJets]
	...
	## Apply jet selection
        ...
        setattr(event,"rho_fatJets"               +self.cfg_ana.collectionPostFix, self.rho                    ) 
        setattr(event,"deltaMetFromJEC_fatJets"   +self.cfg_ana.collectionPostFix, self.deltaMetFromJEC        ) 
        setattr(event,"allJetsUsedForMET_fatJets" +self.cfg_ana.collectionPostFix, self.allJetsUsedForMET      ) 
        setattr(event,"fatJets"                   +self.cfg_ana.collectionPostFix, self.jets                   ) 
        setattr(event,"fatJetsFailId"             +self.cfg_ana.collectionPostFix, self.jetsFailId             ) 
        setattr(event,"fatJetsAllNoID"            +self.cfg_ana.collectionPostFix, self.jetsAllNoID            ) 
        setattr(event,"fatJetsIdOnly"             +self.cfg_ana.collectionPostFix, self.jetsIdOnly             ) 
        setattr(event,"cleanFatJetsAll"           +self.cfg_ana.collectionPostFix, self.cleanJetsAll           ) 
        setattr(event,"cleanFatJets"              +self.cfg_ana.collectionPostFix, self.cleanJets              ) 
        setattr(event,"cleanFatJetsFwd"           +self.cfg_ana.collectionPostFix, self.cleanJetsFwd           ) 
        setattr(event,"discardedFatJets"          +self.cfg_ana.collectionPostFix, self.discardedJets          ) 
        setattr(event,"gamma_cleanFatJetsAll"     +self.cfg_ana.collectionPostFix, self.gamma_cleanJetsAll     ) 
        setattr(event,"gamma_cleanFatJets"        +self.cfg_ana.collectionPostFix, self.gamma_cleanJets        ) 
        setattr(event,"gamma_cleanFatJetsFwd"     +self.cfg_ana.collectionPostFix, self.gamma_cleanJetsFwd     ) 


	if self.cfg_comp.isMC:
            setattr(event,"cleanGenFatJets"                +self.cfg_ana.collectionPostFix, self.cleanGenJets           ) 
            setattr(event,"bqObjects_fatJets"              +self.cfg_ana.collectionPostFix, self.bqObjects              ) 
            setattr(event,"cqObjects_fatJets"              +self.cfg_ana.collectionPostFix, self.cqObjects              ) 
            setattr(event,"partons_fatJets"                +self.cfg_ana.collectionPostFix, self.partons                ) 
            setattr(event,"heaviestQCDFlavour_fatJets"     +self.cfg_ana.collectionPostFix, self.heaviestQCDFlavour     ) 
            setattr(event,"deltaMetFromJetSmearing_fatJets"+self.cfg_ana.collectionPostFix, self.deltaMetFromJetSmearing) 
            setattr(event,"genFatJets"                     +self.cfg_ana.collectionPostFix, self.genJets                ) 
...
setattr(FatJetAnalyzer,"defaultConfig", cfg.Analyzer(
    class_object = FatJetAnalyzer,
    jetCol = 'slimmedJets',
    copyJetsByValue = False,      #Whether or not to copy the input jets or to work with references (should be 'True' if FatJetAnalyzer is run more than once)
    ...
    cleanSelectedLeptons = True, #Whether to clean 'selectedLeptons' after disambiguation. Treat with care (= 'False') if running FatJetAnalyzer more than once
    minLepPt = 10,
